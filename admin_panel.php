...
if (isset($_POST['submit'])) {
  ...
  if (empty($_FILES['pimage']['name'])) {
    ...
    $targetDir = 'image/product/catagory/' . $catagory . '/';
    // echo $targetDir;
    if (!is_dir($targetDir)) {
  ...
  if (empty($pnameErr) && empty($ptypeErr) && empty($brandErr) && empty($pimageErr) && empty($catagoryErr) && empty($quantityErr) && empty($priceErr)) {
    $checkProduct_nameQuery = "SELECT COUNT(*) as count from product where product_name='$pname'";

    $result = mysqli_query($conn, $checkProduct_nameQuery);
    $row = mysqli_fetch_assoc($result);

    if ($row['count'] > 0) {
      echo '<div class="alert alert-primary" role="alert">
      Product already on database!
    </div>';
    } else {
